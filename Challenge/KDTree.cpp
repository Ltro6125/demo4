...
void findNearestCityRec(Node* root, vector<Point>& points, City& nearestCity, double& minTotalDistance, unsigned depth) {
    if (root == NULL) {
    ...
    unsigned cd = depth % 2;
    double distToCurrentPoint = (cd == 0) ? root->city.point.x : root->city.point.y;
    ...
    // Decide which subtrees to explore based on the bounding box
    bool toLeft = false, toRight = false;

    if (cd == 0) { // Compare by latitude (x)
        if (minX < distToCurrentPoint) { // If any point is to the left of the current node
            toLeft = true;
        }
        if (maxX > distToCurrentPoint) { // If any point is to the right of the current node
            toRight = true;
        }
    }
    else { // Compare by longitude (y)
        if (minY < distToCurrentPoint) { // If any point is below the current node
            toLeft = true;
        }
        if (maxY > distToCurrentPoint) { // If any point is above the current node
            toRight = true;
        }
    }

    // Explore the subtree that is closer to the current point
    if (toLeft) {
        findNearestCityRec(root->left, points, nearestCity, minTotalDistance, depth + 1);
    }
    if (toRight) {
        findNearestCityRec(root->right, points, nearestCity, minTotalDistance, depth + 1);
    }
}
...
City nearestNeighborSearch(Node* root, vector<Point> coordinates) {
    ...
    findNearestCityRec(root, coordinates, nearestCity, minTotalDistance, 0);  // Start the recursive search
